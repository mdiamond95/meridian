...
**You do**
1. ~~Pick the first three templates to encode by hand.~~ *Amended 2026-09-17: there are no hand-drawn templates and there will not be. The three shipped presets are generated by the engine (Sitting B, item 6).*
2. ~~Run the same seed on iPad and desktop and confirm identical output.~~ *Amended 2026-09-17: replaced by the cross-engine determinism test — one fixed seed and params (Alberta, bisect, N=15; Canada, equal population, N=10) run in Chromium, WebKit and Firefox must give identical assignment hashes (`npm run determinism -w app`, in CI).*
...
6. (Amended 2026-09-17; no hand-drawn templates.) Three presets generated by the engine and
   committed as packs under app/public/packs/ (moved to the top-level packs/ in Phase 5), each recording its seed and params so it can be
   regenerated: alberta-15 (bisect, lens = Economic preset), canada-26 (bisect, lens =
   Internal-colony index), canada-14 (seeded growth from today's provincial and territorial
   capitals). The presets are the test cases for the loader, the pack library, sharing links and
   re-fit across mesh versions. No "draft until verified" badge.
```

**Gate (amended 2026-09-17):** golden hashes committed; every region contiguous under hard mode; the cross-engine determinism test passes (identical assignment hashes in Chromium, WebKit and Firefox for the fixed cases); N=1 through N=30 on Canada produce no crash and no non-contiguous region under hard mode. Tag `pre-phase4`, then `v0.3-splitter`.
...
**You do**
1. ~~Import one exported KML into Google My Maps and one GeoJSON into the cities atlas to prove the round trip.~~ *Amended 2026-09-24: replaced by tests. The KML validates against the OGC 2.2 schema and re-parses through @tmcw/togeojson to polygons whose cell assignment matches the original; a Playwright test loads a pack into the twenty-line vanilla Leaflet snippet in docs/interop.md and asserts the region count and a hover label.*
2. ~~Decide the shared pack location.~~ *Decided 2026-09-24: a top-level `packs/` folder in this repo, each pack named `<slug>.<meshVersion>.json`; the app fetches them by relative URL, other projects by raw.githubusercontent.com URL (docs/interop.md).*
...
**Gate (amended 2026-09-24):** the KML validates against the OGC schema and re-parses to the original cells; the docs/interop.md Leaflet snippet draws a pack with the right region count and hover label; re-fit onto a synthetic v2 mesh (1% of cells dropped, 1% added) keeps over 98% of alberta-15's population in the same region; the Markdown export follows the Region panel, ⟨draft⟩ marks kept; the page works with IndexedDB unavailable. Tag `pre-phase6`, then `v0.5-interop`.
...
*Amended for release 1.0.1: Phase 7 shipped no-1912-extensions and a Maritime Union scenario. Acadian Maritimes is no longer a scenario: it is the preset pack `acadie-2`, the three Maritime provinces split on French mother tongue into Acadie and the Maritimes.*
